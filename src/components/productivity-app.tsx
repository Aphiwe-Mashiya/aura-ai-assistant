...
function EmailGenerator({ state, setState }: { state: SavedState; setState: React.Dispatch<React.SetStateAction<SavedState>> }) {
  ...
  const generate = () => {
    if (!email.recipient || !email.subject || !email.purpose) return toast.error("Add a recipient, subject, and purpose first.");
...
function TaskPlanner({ tasks, setTasks }: { tasks: Task[]; setTasks: (tasks: Task[]) => void }) {
  ...
  const add = () => { if (!title.trim()) return toast.error("Name the task you want to plan."); setTasks([...tasks, { id: Date.now(), title: title.trim(), duration, urgency, importance, done: false }]); setTitle(""); toast.success("Task added and prioritized."); };
  const optimize = () => { if (!tasks.length) return toast.error("Add a task before optimizing your plan."); setLoading(true); setTimeout(() => { setTasks(sorted); setLoading(false); toast.success(`${mode} plan optimized around impact and urgency.`); }, 700); };
